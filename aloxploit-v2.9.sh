...
echo "  Herramientas utilizadas: Metasploit, Shellter, Setoolkit, Sqlmap, Nmap, Ettercap"
...
#EN ESTE APARTADO ME SALE UN ERROR QUE NO SE PUDO COMPLETAR POR PAQUETES ROTOS (el problema era porque tenia puesta la version y ya hay superiores)
	...
	which shellter >/dev/null 2>&1 && 
        ...
        which bettercap >/dev/null 2>&1 && 
                ...
                apt-get install bettercap -y >> /dev/null
...
# ERROR, NO APARECE NI EL MENU, SE QUEDA PILLADO (arreglado)
	...
	which msfconsole >/dev/null 2>&1 && 
        ...
        which jarsigner >/dev/null 2>&1 && 
                echo "jarsigner : Encontrado!" >> /dev/null || 
                echo "Instalando openjdk-jarsigner..."
                apt-get install openjdk-11-jdk -y >> /dev/null

        which apktool >/dev/null 2>&1 && 
                echo "apktool : Encontrado!" >> /dev/null || 
